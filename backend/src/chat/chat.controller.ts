...
class MessagesQuery extends MaxId { @IsOptional() @Type(() => Number) @IsInt() peer_user_id?: number }
...
export class ChatController {
  ...
  @Get('students') students(@Query() query: MaxId, @CurrentMaxUser() user: MaxUser) { assertMaxUserId(query.max_user_id, user); return { ok: true, data: { students: this.chat.students() } } }
