...
export function AdminWorkspacePage() {
  ...
  const createStudent = async (event:FormEvent) => {
    ...
    const saved = await run('/admin/students/create',{target_max_user_id:Number(studentMaxId),full_name:name,phone,metro,lessons_count:Number(lessons),status:'studying'})
    if (saved) { setName(''); setPhone(''); setMetro(''); setStudentMaxId('') }
  }
  const createTeacher = async (event:FormEvent) => {
    event.preventDefault()
    const saved = await run('/admin/teachers',{full_name:teacherName,target_max_user_id:Number(teacherMaxId),action:'assign'})
    if (saved) { setTeacherName(''); setTeacherMaxId('') }
  }
  return <section className="fi">
    ...
    <div className="admin-columns">
      ...
      <form className="tool-form" onSubmit={createStudent}><h3>Новый ученик</h3><label>MAX ID<input inputMode="numeric" value={studentMaxId} onChange={(e)=>setStudentMaxId(e.target.value)} required /></label><label>Имя<input value={name} onChange={(e)=>setName(e.target.value)} required /></label><label>Телефон<input value={phone} onChange={(e)=>setPhone(e.target.value)} required /></label><label>Метро<input value={metro} onChange={(e)=>setMetro(e.target.value)} /></label><label>Уроков<input type="number" min="1" value={lessons} onChange={(e)=>setLessons(e.target.value)} /></label><button className="button primary" disabled={busyAction!==null} type="submit">{busyAction==='/admin/students/create'?'Добавляем…':'Добавить ученика'}</button></form>
