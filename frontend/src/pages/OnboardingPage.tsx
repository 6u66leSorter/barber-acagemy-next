...
export function OnboardingPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    } catch { setMessage('Не удалось отправить данные. Проверьте заполнение полей и попробуйте ещё раз.') }
  ...
  return <section className="onboarding fi">
    ...
    <div className="segmented" role="tablist" aria-label="Выбор роли">
      <button type="button" className={mode === 'student' ? 'active' : ''} onClick={() => setMode('student')}>Я ученик</button>
      <button type="button" className={mode === 'teacher' ? 'active' : ''} onClick={() => setMode('teacher')}>Я преподаватель</button>
    </div>
