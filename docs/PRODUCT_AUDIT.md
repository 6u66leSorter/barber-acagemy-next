...
| Onboarding | Без роли | Регистрация/заявка | Подтверждение контакта MAX | Подмена телефона и неизвестный номер | critical | Сервер проверяет HMAC; неизвестный номер остаётся гостем | **done** — unit + API smoke |
| Доступ ученика | Без роли | Создание профиля | Назначение администратором по телефону | Ручной ввод/обход | critical | Только подписанный контакт активирует заранее назначенную роль | **done** — API smoke, старый маршрут `410` |
| Доступ преподавателя/админа | Без роли | Заявка/ручной MAX ID | Назначение администратором по телефону | Первый админ и конфликт номера | critical | `INITIAL_ADMIN_PHONE`; уникальная привязка номера; audit | **done** — unit + API smoke |
...
Фактический результат обновляется после финального прогона текущей версии. Проверяются Jest, API smoke, lint, production build, Compose и Docker health; отдельно в реальном мобильном MAX необходимо подтвердить нативное окно `requestContact`.
